...
describes what users get. Pick a format with `-o`, or let uniref choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
uniref <command> -o table   # aligned columns for reading
uniref <command> -o jsonl   # one JSON object per line, for piping
uniref <command> -o json    # a single JSON array
uniref <command> -o csv     # spreadsheet friendly
uniref <command> -o tsv     # tab-separated
uniref <command> -o url     # just the URL column
uniref <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
uniref <command> --fields id,title,url
```
...
```bash
uniref <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
uniref <command>            # a table, because this is a terminal
uniref <command> | wc -l    # JSONL, because this is a pipe
```
